...
const url = 'http://localhost:8090/api/verizon/getLoginDataOutputList';

export default class Verizon extends React.Component {
    ...
    componentDidMount() {
        axios.get(url).then(response => {
            ...
            this.dataInNYTime = [];
            this.yVerizon = ["Users", 0, 0, 0, 0, 0, 0, 0];
            this.total_cts = 0;
            this.calculate();
    ...
    componentDidUpdate(prevProps) {
        if ((prevProps.startdatepropval !== this.props.startdatepropval) || (prevProps.enddatepropval !== this.props.enddatepropval)) {
            ...
            //this.user_login_data = require("../data/user_login_data_verizon");
            axios.get(url).then(response => {
                console.log(response.data);
                this.user_login_data = response.data;
                this.weekday = ["Weekdays", "Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
    ...
    calculate = () => {
        ...
        for (let i = 0; i < this.user_login_data.length; i++) {
            ...
            if (this.dataInNYTime[i].format("YYYY-MM-DD") >= this.props.startdatepropval && this.dataInNYTime[i].format("YYYY-MM-DD") <= this.props.enddatepropval) {
                if (this.dataInNYTime[i].format("dddd") == this.weekday[1])
                    this.yVerizon[1] += this.user_login_data[i].count;
                else if (this.dataInNYTime[i].format("dddd") == this.weekday[2])
                    this.yVerizon[2] += this.user_login_data[i].count;
                else if (this.dataInNYTime[i].format("dddd") == this.weekday[3])
                    this.yVerizon[3] += this.user_login_data[i].count;
                else if (this.dataInNYTime[i].format("dddd") == this.weekday[4])
                    this.yVerizon[4] += this.user_login_data[i].count;
                else if (this.dataInNYTime[i].format("dddd") == this.weekday[5])
                    this.yVerizon[5] += this.user_login_data[i].count;
                else if (this.dataInNYTime[i].format("dddd") == this.weekday[6])
                    this.yVerizon[6] += this.user_login_data[i].count;
                else if (this.dataInNYTime[i].format("dddd") == this.weekday[7])
                    this.yVerizon[7] += this.user_login_data[i].count;
            }
        }

        this.chart_verizon = c3.generate({
            bindto: '#chart_verizon',
            data: {
                columns: [
                    this.yVerizon
                ]
        ...
        this.hours_verizon = c3.generate({
            bindto: '#hours_verizon',
            data: {
                columns: [
                    this.sun_hours,
                    this.mon_hours,
        ...
        if (this.props.startdatepropval == null || this.props.enddatepropval == null) {
            ...
            return (
                <div>
                    ...
                    <h2>Verizon (Users vs Weekdays)</h2>
                    <div id="chart_verizon"></div>
                    <br /><br />
                    <h2>Verizon (Users vs Hours)</h2>
                    <div id="hours_verizon"></div>
                    <br /><br />
                </div>
            );
        }
